...
public class DAO {
	...
	// câu lệnh sql truyền vào 1 hay '1' đều hiểu
	public List<Product> getAllProductByCID(String cid) {
		...
		try {
			conn = new DBContext().getConnection();// mo ket noi voi sql
			ps = conn.prepareStatement(query);
			ps.setString(1, cid); // gán cid vào trong dấu chấm hỏi thứ nhất
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public List<Product> searchByName(String txtSearch) {
		...
		try {
			conn = new DBContext().getConnection();// mo ket noi voi sql
			ps = conn.prepareStatement(query);
			ps.setString(1, "%" + txtSearch + "%"); // gán cid vào trong dấu chấm hỏi thứ nhất
			rs = ps.executeQuery();
			while (rs.next()) {
				list.add(new Product(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getString(5),
						rs.getString(6)));
	...
	public List<Product> getAllProductBySellID(int cid) {
		List<Product> list = new ArrayList<>();
		String query = "select * from product\n" + "where sell_ID = ? ";
		try {
			conn = new DBContext().getConnection();// mo ket noi voi sql
			ps = conn.prepareStatement(query);
			ps.setInt(1, cid); // gán cid vào trong dấu chấm hỏi thứ nhất
			rs = ps.executeQuery();
			while (rs.next()) {
				list.add(new Product(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getDouble(4), rs.getString(5),
						rs.getString(6)));
	...
	public void deleteProduct(String pid){
		...
		try {
			conn = new DBContext().getConnection();// mo ket noi voi sql
			ps = conn.prepareStatement(query);
			ps.setString(1, pid); // gán cid vào trong dấu chấm hỏi thứ nhất
			ps.executeUpdate();
		} catch (Exception e) {
